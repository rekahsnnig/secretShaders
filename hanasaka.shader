Shader "geometry/Flower"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            [maxvertexcount(81) ]
            void geom(triangle d2g input[3], inout TriangleStream<g2f> OutputStream)
            {
                g2f v = (g2f)0;
               ...
               // po.xyz += n * simplex3d(po)/10.;
                float3 r3 = floor(random3(po)*1000.)/1000.;
                ...
                for(int h = 0; h < LAYER ;h++)
                {
                    ...
                    {
                        float rad = (360./CYCLE + _A * sin((_Time.y + length(p.xyz - p.zxy)*15. ) * _ROT)) * (j + 1.) * (UNITY_PI/180.) + _PESL * (h + 1.);
                        float layerA = ((100./LAYER + _A * sin((_Time.y + length(p.xyz - p.zxy)*15. ) * _MOVE)) * (h + 1.)) * (UNITY_PI/180.);
                        [unroll]
                        for(int i = 0; i < 4; i++)
                        {
            ...
            fixed4 frag (g2f i) : SV_Target
            {
                ...
                clip(( length(i.uv )>_TH || c < (1 - _AMOUNT)) * -1.);
