Shader "Unlit/Crystal_tess_h_color"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			 ...
			 void geom(triangle d2g ip[3], inout TriangleStream<g2f> OutputStream)
			{
				...
				float3 size1 = (random - 0.5)*_CrystalSize;
				float3 size2 = (rand(size1) - 0.5) * _CrystalSize;
				
				float len =_CrystalLength * length(random);
			...
			//	float hr = lerp(perlinNoise(float2(i.uv - float2(i.h,i.h) * 100)) , 1,abs(sin(i.h)));
				...
				ihr =(ihr*rand(floor(float3(i.h,i.uv)*100)))/2 + (perlinNoise(float2(length(i.uv*100),i.h)) )*1.1;
				ihr = clamp(ihr,0,1);
				...
				float3 ccolor = float3(1,1,1) * _Color * (1 - ihr);

				fixed3 col = Crystal * ccolor + nCrystal * tex2D(_MainTex, i.uv).rgb;
				float rate = 0.3;

				float3 specularCol =  Crystal * lerp(_Color,1,0.5)  + nCrystal * float3(1,1,1);
	
				float3 CrystalCol = reflection(i,col,specularCol,11) ;
			//	CrystalCol *= 1.1;
