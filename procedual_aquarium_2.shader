    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            data Fish(float3 p,float scale)
            {
                p = p/scale;
                data o = (data)100;
                p.z += sin(_Time.y*1.3 + p.x * 7.)/25.; // swim
                float3 body = p;
            ...
            data map(float3 p)
            {
               ...
               // sand.y += (0.5-vnoise(sand.xz * 3.))/10.;
                ...
                float fishid = rand(floor(fish.xz * 3.));
                fish.xz = mod(fish.xz,3.) - 1.5;
                ...
                float ofs = kelpcell * 16.1;
                kelp.y += 0.4;
                kelp.y -= (_KelpScale * kelpcell)/2.;
            ...
            float3 render(data d,float3 ro,float3 rd,float3 cd)
            {
                ...
                float softshadow = smoothstep(0.,1.,shadow/10.);
