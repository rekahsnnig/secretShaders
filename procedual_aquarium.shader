        ...
        Pass
        {
           ...
           //     fish = mul(RotMat(float3(0.,1.,0.),-_Time.y/3.),fish)- float3(0.,0.,0.6);
            ...
            float wsurf(float3 p,float3 n)
            {
                ...
                //float wsurfray = step(fBm(fBm(p.xz- float2(1.1,length(p) * 2.+_Time.y/10.)) * float2(0.1,_Time.y)),0.5 );
                //float wsurfray = step(fBm(p.xz- float2(1.1,length(p) * 2.+_Time.y/10.)),0.54 );
